...
void NextGen(int sierpBoard [][MAX_ARRAY_SIZE], int numRowsInBoard, int numColsInBoard, int generationNum);
...
int main() {
	...
	int mysierpBoard[MAX_ARRAY_SIZE][MAX_ARRAY_SIZE];
	string temp;
	...
	InitGen(mysierpBoard , numRowsInBoard, numColsInBoard, generations);
	
	temp.clear();
	cout<<"Enter the name of output file for Sierpinski Triangle: "<<endl;
	cin>>temp;
	myfout.open(temp);
	if(myfout.fail()) {
		cerr<<"ERROR: output file not opened correctly";
	...
	PrintGen(mysierpBoard, cout, numRowsInBoard, numColsInBoard, 0);	//on console ...generations can be changed later 
	PrintGen(mysierpBoard, myfout, numRowsInBoard, numColsInBoard, 0);	//on file
	
	for(int i=1;i<=generations;i++) {	
		NextGen(mysierpBoard, numRowsInBoard, numColsInBoard, i);
		PrintGen(mysierpBoard, myfout, numRowsInBoard, numColsInBoard, i);
		if(i==1 || i==generations) {
			PrintGen(mysierpBoard, cout, numRowsInBoard, numColsInBoard, i);
		}
	}
	myfout.close();
...
void InitGen(int sierpBoard[][50] , int& numRowsInBoard, int& numColsInBoard, int& generations) {
	string temp;
	ifstream fin;
	...
	fin>>ws;					// to eat up all the whitespaces in the file
	if(isdigit(fin.peek())) {	// to check if the next bit of data is a character
		fin>>numRowsInBoard; 	// reading number of rows
		if(numRowsInBoard<0 || numRowsInBoard>50)
			cerr<<"ERROR: Read an illegal number of rows for the board";
	}
	...
	if(isdigit(fin.peek())) {
		...
		cerr<<"ERROR: Cannot read number of generations";	
		abort();
	}
	
	//10th instruction in main assignmnet
...
void NextGen(int sierpBoard [][MAX_ARRAY_SIZE], int numRowsInBoard, int numColsInBoard, int generationNum) {
	int nextGenBoard[MAX_ARRAY_SIZE][MAX_ARRAY_SIZE];		//initializing local board
	
	for(int i=0;i<MAX_ARRAY_SIZE;i++) {				//initializing all the board to 0
		for(int j=0;j<MAX_ARRAY_SIZE;j++) {
			nextGenBoard[i][j] = 0;
		}
	}
	...
	for(int i=1;i<numRowsInBoard;i++) {
		for(int j=1;j<numColsInBoard;j++) {
			if(i==0 || j==0 || (i==0 && j==0) ) {
				nextGenBoard[i][j] = 0;
			}
			else {
				nextGenBoard[i][j] = ( sierpBoard[i][j] + sierpBoard[i-1][j] + sierpBoard[i][j-1] ) % 2;
			}
		}
	}
	
	//copying into Sierpinski Board:
	for(int i=0;i<numRowsInBoard;i++) {
		for(int j=0;j<numColsInBoard;j++) { 
			sierpBoard[i][j] = nextGenBoard[i][j];
		}
	}
	return;
}
...
void PrintGen(int sierpBoard[ ][MAX_ARRAY_SIZE], ostream& out, int numRowsInBoard,int numColsInBoard, int generationNum) {
	if(generationNum==0) 
		out<<"LIFE initial game board"<<endl;
	else 
		out	<<"LIFE gameboard: generation "<<generationNum<<endl;
